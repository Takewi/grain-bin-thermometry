...
export function buildStorageStructure(
  scene: Scene,
  item: PositionedStorage
): StorageMeshBundle {
  ...
  if (data.type === "SILO") {
    ...
    baseCylinder.position = new Vector3(
      0,
      (SILO.BASE_HEIGHT - baseExtraDepth) / 2,
      0
    );
    ...
    bodyCylinder.position = new Vector3(
      0,
      SILO.BASE_HEIGHT + bodyHeight / 2,
      0
    );
    ...
    roofCone.position = new Vector3(
      0,
      SILO.BASE_HEIGHT + bodyHeight + SILO.ROOF_HEIGHT / 2,
      0
    );
    ...
    for (let i = 0; i < ribCount; i++) {
      ...
      const rib = MeshBuilder.CreateTube(
        ...
        {
          path: [
            new Vector3(x1, topCapY - 0.05, z1),
            new Vector3(x2, bottomEaveY, z2),
          ],
          radius: 0.035,
          tessellation: 8,
        },
    ...
    for (let r = 1; r < rungCount; r++) {
      ...
      const rung = MeshBuilder.CreateTube(
        ...
        {
          path: [
            new Vector3(leftX, rungY, leftZ),
            new Vector3(rightX, rungY, rightZ),
          ],
          radius: 0.018,
          tessellation: 8,
        },
    ...
    for (let y = startY; y <= endY + 0.1; y += cageStep) {
      ...
      for (let s = 0; s <= cageSegments; s++) {
        ...
        const hx = ladderX + tangFactor * (-Math.sin(ladderAngle)) + normFactor * Math.cos(ladderAngle);
        const hz = ladderZ + tangFactor * Math.cos(ladderAngle) + normFactor * Math.sin(ladderAngle);
    ...
    if (cageHoopsY.length >= 2) {
      ...
      [0.25, 0.5, 0.75].forEach((ratio, barIdx) => {
        ...
        const bx = ladderX + tangFactor * (-Math.sin(ladderAngle)) + normFactor * Math.cos(ladderAngle);
        const bz = ladderZ + tangFactor * Math.cos(ladderAngle) + normFactor * Math.sin(ladderAngle);
    ...
    plaqueMat.emissiveColor = new Color3(0.55, 0.60, 0.70);
    ...
    baseBox.position = new Vector3(
      0,
      (WAREHOUSE.BASE_HEIGHT - baseExtraDepth) / 2,
      0
    );
    ...
    ventMonitor.position = new Vector3(
      0,
      baseY + roofHeight + 0.11,
      0
    );
