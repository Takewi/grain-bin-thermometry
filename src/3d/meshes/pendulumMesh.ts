...
function getOrCreateBadgeMaterial(
	...
	const texture = new DynamicTexture(`badge_tex_${cacheKey}`, { width, height }, scene, false);
...
export function renderThermometry(
	...
	if (type === "SILO") {
		...
		sectorZSet.forEach((secZ) => {
			const halfW = dimensions.width * 0.4;
	...
	parsedPendulums.forEach((p) => {
		...
		const pCableTopY = isWh ? cableTopY + archFactor * 0.75 : cableTopY;

		const pSensorBottomY = isWh ? sensorBottomY + archFactor * 0.2 : sensorBottomY;

		const pSensorTopY = isWh ? sensorTopY + archFactor * 0.7 : sensorTopY;
		...
		p.sensors.forEach((sensor, sIdx) => {
			...
			const meta: SensorInstanceMetadata = {
				...
				reading: sensor,
				worldPosition: new Vector3(parentPos.x + p.localX, parentPos.y + y, parentPos.z + p.localZ),
			};
