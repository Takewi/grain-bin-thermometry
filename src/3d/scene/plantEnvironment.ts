...
export function setupPlantEnvironment(scene: Scene): EnvironmentBundle {
  ...
  const hemiBottom = new HemisphericLight(
    "hemiBottom",
    new Vector3(0, -1, 0),
    scene
  );
  ...
  const dirKey = new DirectionalLight(
    "dirKey",
    LIGHTING.DIR_KEY_DIRECTION,
    scene
  );
  ...
  const dirFill = new DirectionalLight(
    "dirFill",
    LIGHTING.DIR_FILL_DIRECTION,
    scene
  );
